...
`main` 分支更新后会构建 `main`、`latest` 和根目录 `VERSION` 对应的版本镜像；正式 release 也会更新 `latest`。如需锁定版本，可在 `.env` 中设置 `IMAGE_TAG`（例如 `0.8.0`）。
...
历史任务支持限定条数或扫描全部历史，并先暂存扫描结果，再按消息从旧到新的顺序发送。任务可以暂停、恢复、取消和删除；数据库会记录已归档的来源消息，重复扫描不会重新复制已经处理过的消息。普通模式下，暂停历史任务不会暂停实时转发；严格时间顺序模式则会安全暂存同期新消息，等历史追平后再统一按来源消息 ID 从旧到新发送。为了不影响实时任务，手工暂停历史扫描会主动解除这段严格暂存；取消严格顺序任务只删除尚未发送的历史补档，不会丢掉同期新消息。

启用“恢复遗漏消息”后，每条路线及 Topic 都会持久保存核对游标。容器重启、网络中断或账号短暂离线后，系统只扫描游标到 Telegram 当前消息之间的缺口，并利用唯一归档记录跳过已经发送的消息。论坛群组之间还可以选择“保留来源主题”：每个来源 Topic 都拥有独立、持久的目标 Topic 映射，即使名称相同也不会再合并；首次创建会复制名称、颜色和可用的自定义图标，之后还会核对名称、关闭状态及 General 的隐藏状态。频道和普通群组不显示 Topic 设置。

云端转发默认采用单账号均衡保护：实时任务最多每分钟 20 条、每小时 500 条、每天 5,000 条；历史补档和断档恢复保留实时余量，最多每分钟 15 条、每小时 450 条、每天 4,500 条，相同路由和主题最多 15 条一批提交。实时消息优先于历史积压；Telegram 返回 `FLOOD_WAIT` 时仍会自动暂停并延后恢复。
