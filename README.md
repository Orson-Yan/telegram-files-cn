...
历史任务支持限定条数或扫描全部历史，并先暂存扫描结果，再按消息从旧到新的顺序发送。任务可以暂停、恢复、取消和删除；数据库会记录已归档的来源消息，重复扫描不会重新复制已经处理过的消息。论坛群组之间还可以选择“保留来源主题”，系统会读取全部 Topic，并在扫描到各 Topic 时按需创建同名目标 Topic，避免大型论坛在扫描开始前长时间停留在 0；来源与目标 Topic 的映射会持久保存。发送前会重新校验主题映射，无法解析目标主题时会明确失败，不会静默落入 General。频道和普通群组不显示 Topic 设置。

云端转发默认采用单账号均衡保护：每分钟 20 条、每小时 500 条、每天 5,000 条；相同路由和主题的普通消息最多 20 条一批提交。实时消息优先于历史积压，不同历史来源会公平轮询；Telegram 返回 `FLOOD_WAIT` 时仍会自动暂停并延后恢复。
